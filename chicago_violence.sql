...
-- 2. What was the most frequently committed crime based on the FBI vicitimization description?
...
-- 6. What is the most dangerous community area?
...
-- 13. What are the ten days the most crimes were committed?
SELECT date, COUNT(date) AS cnt FROM time
GROUP BY date
ORDER BY cnt DESC
LIMIT(10);

-- 14. In which year were the most crimes committed?
...
-- 19. Which age group committed the most crimes?
...
-- 23. Which race committed the most crimes?
SELECT race,COUNT(race) AS cnt FROM details
GROUP BY race
ORDER BY cnt DESC;

-- 24. In which months does each race commit the most crimes?
SELECT month,race,COUNT(*) AS cnt FROM full_
GROUP BY month,race
HAVING race!='UNKNOWN'
ORDER BY race,cnt DESC;

-- 25. What are the numbers of crimes committed by each race in different areas?
